...
export const ChartRenderer: React.FC<ChartRendererProps> = ({ config, excelData, onDownload }) => {
  ...
  const handleDownload = () => {
    if (chartRef.current) {
      ...
      downloadChart(canvas, `${config.title}_chart`);
    }
    onDownload();
  };
  ...
  const renderChart = () => {
    switch (config.type) {
      ...
      case 'scatter':
        return <Scatter ref={chartRef} data={{ datasets: [{ ...chartData.datasets[0], data: labels.map((_, i) => ({ x: i, y: values[i] })) }] }} options={options} />;
      case 'doughnut':
